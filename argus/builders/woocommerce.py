...
* ``woocommerce-init`` — a one-shot ``wordpress:cli`` container that provisions
  ...
  checkout, disables user registration, switches the cart/checkout to the classic
  shortcode pages and strips every checkout field (the buyer is asked for no
  information — see the hardening mu-plugin), imports the cards (reused from
...
def build_woocommerce(ctx: BuildContext) -> Fragment:
    ...
    init = {
        ...
        # Run as uid:gid 33 to match the Debian wordpress image's www-data (the
        # owner of the shared wp-content volume). The wordpress:cli image is
        # Alpine-based, where "www-data" is uid 82 and could not write wp-content.
        "user": "33:33",
        "entrypoint": ["/bin/sh", "/provision/provision.sh"],
        "environment": {
            # The WordPress wp-config.php resolves DB settings from the environment
            # at runtime, so wp-cli needs the same DB vars as the wordpress service
            # (otherwise it falls back to the image defaults and can't connect).
            "WORDPRESS_DB_HOST": "woocommerce-db:3306",
            "WORDPRESS_DB_NAME": "wordpress",
            "WORDPRESS_DB_USER": "wordpress",
            "WORDPRESS_DB_PASSWORD": "${WORDPRESS_DB_PASSWORD}",
...
set -eu
export HOME=/tmp  # wp-cli writes its cache under $HOME; uid 33 can't write /
WP="wp --path=/var/www/html"
log() { echo "[provision] $*"; }

# 1. Wait for the WordPress core files (copied by the wordpress service on its
# first start). We gate on the files only — NOT `wp db check`, whose mariadb
# client defaults to requiring TLS the server doesn't offer; the actual install
# below connects over PHP/mysqli, which has no such default.
i=0
while [ "$i" -lt 60 ]; do
    if [ -f /var/www/html/wp-load.php ] && $WP core version >/dev/null 2>&1; then
        break
    fi
    [ "$i" = 0 ] && log "waiting for WordPress core files..."
    i=$((i + 1)); sleep 5
done
...
# Don't prompt for login at checkout — that's a request for credentials, and the
# storefront sells to guests only (registration is off).
$WP option update woocommerce_enable_checkout_login_reminder no
# No public content from non-admins: turn off product reviews entirely.
$WP option update woocommerce_enable_reviews no
$WP option update woocommerce_review_rating_verification_required yes || true
...
$WP option update woocommerce_extended_task_list_hidden yes || true
...
# 4b. Use the CLASSIC (shortcode) cart/checkout pages. WooCommerce 10.x creates
# these as block pages, and the checkout block hardcodes a required email field
# that cannot be removed. The classic [woocommerce_checkout] shortcode renders
# whatever fields the `woocommerce_checkout_fields` filter leaves — and the
# argus-hardening mu-plugin (step 7) strips every one of them — so checkout asks
# the buyer for nothing. Overwrites the page body each run (idempotent), which
# also converts an already-deployed block-checkout store on the next provision.
for pair in "woocommerce_cart_page_id:[woocommerce_cart]" \
            "woocommerce_checkout_page_id:[woocommerce_checkout]"; do
    opt=${pair%%:*}; shortcode=${pair#*:}
    pid=$($WP option get "$opt" 2>/dev/null || echo "")
    if [ -n "$pid" ] && [ "$pid" != "0" ]; then
        $WP post update "$pid" --post_content="$shortcode" >/dev/null \
            && log "set page $pid to $shortcode"
    fi
done
...
# and closes every path by which a non-admin could add content (comments, product
# reviews, public registration). Kept conservative so guest checkout (classic
# cart/checkout) keeps working.
_HARDENING_MU_PLUGIN = r"""<?php
/**
 * Plugin Name: Argus Hardening
 * Description: Generated by Bitcoin Argus — disables unused WordPress features and
 *   every non-admin content path (comments, reviews, registration), and removes
 *   all checkout fields so guests are asked for no information, on the demo
 *   storefront.
 */
if (!defined('ABSPATH')) { exit; }

// Never allow public user registration, regardless of the stored option (so a
// stray option change can't re-open it). Guest checkout does not create users.
add_filter('option_users_can_register', '__return_zero');
add_filter('pre_option_users_can_register', '__return_zero');

// Disable WooCommerce product reviews (which are stored as comments): hide the
// reviews tab and force reviews off even if the option is flipped.
add_filter('woocommerce_product_tabs', function ($tabs) {
    unset($tabs['reviews']);
    return $tabs;
}, 98);
add_filter('option_woocommerce_enable_reviews', function () { return 'no'; });
...
// --- Frictionless guest checkout: ask the buyer for NO information ----------
// The store sells virtual demo cards paid in Bitcoin; there is nothing to ship
// and no account to create. Strip every field the classic [woocommerce_checkout]
// would render (billing, shipping, account, order notes) so the checkout shows
// only the cart total and the Pay button. Runs at a late priority so it wins
// over WooCommerce's own field setup.
add_filter('woocommerce_checkout_fields', function ($fields) {
    foreach (array('billing', 'shipping', 'account', 'order') as $group) {
        $fields[$group] = array();  // keep the group key, drop all its fields
    }
    return $fields;
}, 9999);

// Drop the "Additional information" / order-notes section entirely.
add_filter('woocommerce_enable_order_notes_field', '__return_false');

// With no fields collected, give the order a stable placeholder email so the
// order record and the BTCPay/Cashu invoice are well-formed. is_email() requires
// a dotted domain; example.com is the RFC 2606 reserved documentation domain.
// The buyer never sees or types this.
add_filter('woocommerce_checkout_posted_data', function ($data) {
    if (empty($data['billing_email'])) {
        $data['billing_email'] = 'guest@example.com';
    }
    return $data;
}, 9999);

// Belt-and-suspenders: even though the fields are stripped, the classic checkout
// template still prints the (now empty) "Billing details" column wrapper. Hide
// the whole #customer_details block on the checkout page so nothing but the order
// review + payment remains. Scoped to the checkout page only.
add_action('wp_head', function () {
    if (function_exists('is_checkout') && is_checkout() && !is_wc_endpoint_url('order-received')) {
        echo "<style>#customer_details{display:none !important;}</style>\n";
    }
});
"""
