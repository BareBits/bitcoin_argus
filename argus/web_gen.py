...
def _dockerfile() -> str:
    return (
        ...
        # Both base-image ARGs are declared in the global scope (before the first
        # FROM) so each FROM can reference them.
        "ARG EMSDK_IMAGE=emscripten/emsdk:3.1.61\n"
        "ARG PYTHON_IMAGE=python:3.12-slim\n"
        # Stage 1: compile the faucet's yespower proof-of-work primitive to a
        # standalone WASM module (used identically by the browser solver and the
        # server-side verifier). Built in CI rather than committed as a binary.
        "FROM ${EMSDK_IMAGE} AS wasmbuild\n"
        "COPY argus/web/wasm /build/wasm\n"
        "RUN OUT_DIR=/out sh /build/wasm/build-wasm.sh\n"
        "\n"
        "FROM ${PYTHON_IMAGE}\n"
        "WORKDIR /app\n"
        "COPY argus/web/requirements.txt /tmp/web-requirements.txt\n"
        # wasmtime runs the same yespower.wasm the browser does, so the faucet can
        # verify a proof-of-work without a native build or node credentials.
        "RUN pip install --no-cache-dir 'pydantic>=2' 'PyYAML>=6' 'wasmtime>=20' \\\n"
        "    && pip install --no-cache-dir -r /tmp/web-requirements.txt\n"
        "COPY argus /app/argus\n"
        # Drop the CI-built PoW module in beside the faucet's static assets.
        "COPY --from=wasmbuild /out/yespower.wasm /app/argus/web/static/yespower.wasm\n"
